...
//+ Есть 4 методов контекстного вызова this.
...
// *** 4) Ручная привязка к this. 'call', 'apply'
...
//. Отличие только в синтаксисе написания.
//. Присваиваем контекст в старую функцию.
// newUser.call(user, 'John');
// newUser.apply(user, ['John']);

// ---

// **** 4) Ручная привязка к this. 'bind'

// function count(num) {
//   return this * num;
// }

//. В эту переменную помещается новая функция.
...
//. Стрелочная функция будет вызывать не сам объект а его родителя.
...
const obj = {
  num: 5,
  mat: 74,
  //. 2. Контекст sayNumber ссылается на объект obj в котором он существует.
  sayNumber: function () {
    const say = () => {
      //. 1. Стрелочная функция с this всегда ссылается на родителя, родитель это sayNumber.
      console.log(this); //. 3. Итог того что в этом примере this ссылается на obj.
      console.log(this.num); //. 4. this обратилась к объекту а потом к num и вывела в консоль (5).
    };
    say();
  },
};
