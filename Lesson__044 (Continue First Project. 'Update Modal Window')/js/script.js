...
window.addEventListener('DOMContentLoaded', () => {
  ...
  //* Открытие модального окна при скроле в самый низ.
  window.addEventListener('scroll', showModalByScroll);

  //* Автоматическое Открытие модального окна через заданное время.
  const modalTimerId = setTimeout(openModalWindow, 60000); // 1 минута.
  ...
  function openModalWindow() {
    ...
    //. Если модальное окно открывалось раньше чем истечет время то оно не будет открываться повторно.
  ...
  function showModalByScroll() {
    ...
    if (clientScroll >= pageHeight) {
      openModalWindow();
      //. Удаляем обработчик события, что бы он не сработал второй раз.
